...
void print(void) {
    int i, j;
    int part_number[num_parts];

    for(i = 0; i < num_parts; i++)
        part_number[i] = inventory[i].number;
    quicksort(part_number, 0, num_parts - 1);

    int *Lowest_part_number = part_number;

    printf("Part Number     Part Name                   Quantity on Hand\n");
    for(i = 0; i < num_parts; i++) {
        for(j = 0; j < num_parts; j++) {
            if(inventory[j].number == *Lowest_part_number) {
                printf("%7d         %-25s%11d\n", inventory[j].number, inventory[j].name, inventory[j].on_hand);
                Lowest_part_number++;
            }
        }
    }
